...
import { runGovDataPreprocessingPipeline } from "../lib/data-pipeline"
...
app.get("/api/government-data/sync", async (_req: Request, res: Response) => {
  try {
    if (!cachedSyncState) {
...
app.post("/api/government-data/sync", async (req: Request, res: Response) => {
  try {
    const body = req.body || {}
